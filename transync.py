...
if ls_flag:
    with open("MSE_ls_transync_alpha_ST_"+str(alpha)+".txt", "wb") as mse_ls:
        pickle.dump(MSE_ls, mse_ls)
    with open("data_ls_transync_alpha_ST_"+str(alpha)+".txt", "wb") as Data_ls:
        pickle.dump(data_ls, Data_ls)
        
if dls_flag:
    with open("MSE_dls_transync_alpha_ST_"+str(alpha)+".txt", "wb") as mse_dls:
        pickle.dump(MSE_dls, mse_dls)
    with open("data_dls_transync_alpha_ST_"+str(alpha)+".txt", "wb") as Data_dls:
        pickle.dump(data_dls, Data_dls)

if proj_flag:
    with open("MSE_proj_transync_alpha_ST_"+str(alpha)+".txt", "wb") as mse_proj:
        pickle.dump(MSE_proj, mse_proj)
    with open("data_proj_transync_alpha_ST_"+str(alpha)+".txt", "wb") as Data_proj:
        pickle.dump(data_proj, Data_proj)